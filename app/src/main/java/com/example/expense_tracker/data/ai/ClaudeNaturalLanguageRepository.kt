...
class ClaudeNaturalLanguageRepository internal constructor(
    ...
    override suspend fun parse(request: NaturalLanguageRequest): List<ParsedTransaction> {
    ...
    private fun systemPrompt(request: NaturalLanguageRequest): String {
        ...
        return """
            Parse one or more transactions (expense or income) from Indonesian or English natural language. Return JSON ONLY.
            ...
            Return JSON with a "transactions" array containing one or more transaction objects, with no Markdown, comments, explanations, or extra fields:
            {"transactions":[{"amount":25000,"type":"EXPENSE","category":"exact name from categories","merchant":"Warteg","date":"YYYY-MM-DD","note":"makan siang","is_recurring":false}]}
            ...
            category: for each transaction, select exactly one category name from the provided categories list that matches the transaction type (matching category type or "BOTH"), preserving its spelling. Never create a category.
            ...
            Multiple separate transactions mentioned in one sentence (e.g., "beli kopi 25rb lalu makan 50rb dan bensin 30rb") MUST be extracted as separate items in the "transactions" array.
            For transfers between wallets or non-transaction requests, return {"error":"unsupported_transaction"} only.
